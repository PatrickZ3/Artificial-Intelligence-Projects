...
%%%%% NAME: Patrick Santoso Lay
		...
		/* % cps721, 5th assignment, Part 1: file  initTire.pl   %*/
			/* Initial state */
nut(nuts1).	nut(nuts2).	nut(nuts3).	
nut(nuts4).
hub(hub1).	hub(hub2).	hub(hub3).	
hub(hub4).
% wheel5 is a spare wheel that is inflated and can be found inside the trunk
wheel(wheel1).	wheel(wheel2).	wheel(wheel3).	
wheel(wheel4).	
wheel(wheel5).
...
goal_state(69,S) :- inside(wheel4,trunk,S).%on(wheel5,hub4,S),  inside(wheel4,trunk,S).
...
goal_state(100,S) :- on(wheel5,hub4,S), not lifted(hub4,S), .

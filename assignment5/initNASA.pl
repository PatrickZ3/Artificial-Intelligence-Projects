...
%%goal_state(S) :- pointsTo(sat1,comet2,S).

%%% up - powered(instr1, sat1, S).
%%% turnTo - pointsTo(sat1,ground17,S).
%%% runCalibrate - calibrated(instr1,sat1,S).
%%% turnTo - pointsTo(sat1,comet2,S).
%%% takeImage (hasImage)
